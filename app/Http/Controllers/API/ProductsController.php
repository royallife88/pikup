...
class ProductsController extends Controller
{
    ...
    public function all_product_goods()
    {   // fethcing all goods from db
        $products = Laravelproject_new_add_product::get();
    ...
    public function single_good(Request $request)
    {   // validating reuqest
        ...
        return new ProductResource(Laravelproject_new_add_product::where('p_id', $id)->first());
    }

    // all goods by a store
    public function goods_by_store(Request $request)
    {   // validating request
        if (empty($request->all())) {
            return response()->json(['error' => 'Parameters are missing', 'code' => '400'], 400);
        }
        ...
        $prodcuts = Laravelproject_new_add_product::where('store_id', $store_id)->get();
    ...
    public function filter_goods(Request $request)
    {   // validating request
        if (empty($request->all())) {
            return response()->json(['error' => 'Parameters are missing', 'code' => '400'], 400);
        }
        ...
        // filtering goods
        if (!empty($title)) {
            $prodcuts = Laravelproject_new_add_product::where('title', 'LIKE', '%' . $title . '%')->get();
        }
        if (!empty($category)) {
            $prodcuts = Laravelproject_new_add_product::where('category', 'LIKE', '%' . $category . '%')->get();
        }
        if (!empty($price)) {
            $prodcuts = Laravelproject_new_add_product::where('price', 'LIKE', '%' . $price . '%')->get();
        }
